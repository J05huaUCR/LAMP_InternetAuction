...
echo "OUTPUT: |".$_POST['output']."|<br />";

// check for passed in 
(isset($_POST['view']))  		? $view = $_POST['view']   		: $view = 'overview';
...
$datetime = date("F j, Y  g:m:sa", time());
$date = fGetTime($days);
$revenue = 0;
$sql_where = "";
...
switch($view) {		
	case "overview": // Overview
		...
		$stats_datetime = date("F j, Y  g:i:sa", $update_time); // current datetime
		foreach($platforms as $platform){
			$count = fQuery_Single("SELECT COUNT(id) FROM $table_sessions WHERE platform='".$platform."' ", $MYSQL_CONNECTION);
			
			$table .= "<h3>".$PLATFORM_NAMES[$platform]." has $count session(s) out of $total total.</h3>";		
			$sql  = "SELECT ";
			if ($flag == 'range') $sql .= "stats_date as 'Date',";
			$sql .= "feed as 'Feed',SUM(resp_us) as 'US',SUM(resp_intl) as 'Intl',(SUM(resp_us) + SUM(resp_intl)) as 'Total',SUM(num_bids_won) as 'Wins',"; 
			$sql .= "FORMAT(MAX(bid_max),6) as 'Max Bid',FORMAT(MIN(bid_min),6) as 'Min Bid',FORMAT(AVG(bid_avg),6) as 'Avg Bid',FORMAT(AVG(bid_median),6) as 'Median',";
			$sql .= "FORMAT(MIN(resp_fastest),6) as 'Fastest',FORMAT(MAX(resp_slowest),6) as 'Slowest',FORMAT(AVG(resp_avg),6) as 'Avg Time',";
			$sql .= "FORMAT(SUM(rev_us), 2) as 'US $',FORMAT(SUM(rev_intl), 2) as 'Intl $',FORMAT( (SUM(rev_us) + SUM(rev_intl) ),2) as 'Total $' ";
	...
	case "revenue": // Revenue
		...
		$start_wtd = 1345446061;
		$datetime_ytd = date("F j, Y  g:m:sa", $start_ytd); // current datetime
		$datetime_mtd = date("F j, Y  g:m:sa", $start_mtd); // current datetime
		$datetime_wtd = date("F j, Y  g:m:sa", $start_wtd); // current datetime
		//echo "YTD: $start_ytd => $datetime_ytd <br />";
		//echo "MTD: $start_mtd => $datetime_mtd <br />";
		//echo "WTD: $start_wtd => $datetime_wtd <br />";
		$end_date   = time(); // to date
		...
		foreach($platforms as $platform){
			...
			$sql .= "FROM dc_stats_daily WHERE update_time > $start_wtd and platform = '".$platform."' ";
			$sql .= "UNION ";
			$sql .= "SELECT 'Month-To-Date' as Period, (SUM(resp_us) + SUM(resp_intl)) as 'Total Queries',";
			$sql .= "FORMAT(SUM(rev_us),2) as 'US $', FORMAT(SUM(rev_intl),2) as 'Intl $', FORMAT(SUM(rev_us+rev_intl),2) as 'Total $' ";
			$sql .= "FROM dc_stats_daily WHERE update_time > $start_mtd and platform = '".$platform."' ";
			$sql .= "UNION ";
			$sql .= "SELECT 'Year-To-Date' as Period, (SUM(resp_us) + SUM(resp_intl)) as 'Total Queries',";
			$sql .= "FORMAT(SUM(rev_us),2) as 'US $',FORMAT(SUM(rev_intl),2) as 'Intl $',FORMAT(SUM(rev_us+rev_intl),2) as 'Total $' ";
			$sql .= "FROM dc_stats_daily WHERE update_time > $start_ytd and platform = '".$platform."' ";
			//echo "SQL: $sql <br />";
	...
	case "keywords":// Keywords
		...
		foreach($platforms as $platform){
			...
			$sql .= "FORMAT(SUM( CASE WHEN Won > 0 AND Region = 'us' THEN Bid END ),4) as 'Revenue US',";
			$sql .= "FORMAT(SUM( CASE WHEN Won > 0 AND Region != 'us' THEN Bid END ),4) as 'Revenue Intl'";
			$sql .= "FROM ( ";
			for ($i=0; $i < sizeof($dates); $i++) {
				if ($i != 0) $sql .= "UNION ALL ";
	...
	case "redirects":// Redirects
		...
		foreach($platforms as $platform){
			...
			$sql  = "SELECT Domain,Feed,CONCAT('<a href=\"',Link,'\">Link</a>') as 'Redirect Path',COUNT(Link) as 'Redirects',FORMAT(SUM(Bid),4) as Revenue ";
			$sql .= "FROM (";
			for ($i=0; $i < sizeof($dates); $i++) {
				if ($i != 0) $sql .= "UNION ALL ";
