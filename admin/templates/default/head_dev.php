...
function showStats(output) {
	var url = "reports_output<?=$DEV?>.php"; 
	...
	var params = "reports="+whichReport+"&days="+whichDays+"&flag="+days_flag+"&platform="+platform+"&feed="+feed+"&qty="+qty+"&output="+output;
	
	if (output == 1) {
		document.statsreport.submit();
	} else {
		
		// Loading overlay parameters
		var text_height = 0;
		var text_offset = 206;
...
<frameset rows="200,100%">
	...
	var params = "view="+whichReport+"&days="+whichDays+"&flag="+days_flag+"&platform="+platform+"&feed="+feed+"&qty="+qty+"&output="+output;
