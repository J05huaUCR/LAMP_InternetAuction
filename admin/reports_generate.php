...
if ($trace) echo "SQL: $sql<br />";
$DATES = fQuery_Assoc_Multiple($sql, $MYSQL_CONNECTION);*/
...
for ($i = 0; $i < sizeof($FEEDS); ++$i){	
	...
	foreach($PLATFORM_NAMES as $v => $name) {
		if($p) $sql .= "UNION ";
	...
	// check db for stats
	$sql = "SELECT feed FROM $TABLE_STATS WHERE stats_date = '$date_YMD' AND feed='$feed'";
	if ($trace) echo "SQL: $sql <br />";
